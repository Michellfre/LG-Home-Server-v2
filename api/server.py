...
VERSION="Open Home OS v13.3 Discovery UI"
BUILD="000260"
...
TUYA_CFG=CONFIG/"tuya_cloud.json"
TUYA_CACHE=CONFIG/"tuya_cache.json"
NETWORK=CONFIG/"network_devices.json"
DISCOVERY_STATE=CONFIG/"discovery_state.json"
...
def run(c,t=8):
    try:
        return subprocess.check_output(c,shell=True,text=True,stderr=subprocess.DEVNULL,timeout=t).strip()
    except Exception:
        return ""
...
def mac_vendor(mac):
    ...
    vendors={
        "001A11":"Google","F4F5D8":"Google","D8C4E9":"Google",
        "A4CF12":"Espressif","24D7EB":"Espressif","30AEA4":"Espressif","84F3EB":"Tuya/SmartLife",
        "B827EB":"Raspberry Pi","DCA632":"Raspberry Pi","E45F01":"Raspberry Pi",
        "F0D5BF":"Intelbras","001422":"Dell","001B63":"Apple",
        "3C5A37":"Samsung","F45C89":"Samsung","7C2EDD":"Samsung","BC1485":"Samsung",
        "28E31F":"Xiaomi","50EC50":"Xiaomi","A0A3B3":"Sony",
        "E8F2E2":"LG","CC2D83":"LG","ACDCE5":"LG",
        "18B430":"Nest","44D9E7":"Ubiquiti","60E327":"TP-Link","50C7BF":"TP-Link","C0C9E3":"TP-Link",
        "D8B6B7":"Tuya/SmartLife","7CF666":"Tuya/SmartLife","10D561":"Tuya/SmartLife","70A2B3":"Tuya/SmartLife",
...
PORTS={
  ...
  8090:"OpenHomeAPI",1883:"MQTT",5000:"NAS/UPnP",5001:"NAS",8123:"HomeAssistant",9100:"Printer"
}
...
def grab_http(addr,port):
    ...
    try:
        req=urllib.request.Request(url,headers={"User-Agent":"OpenHomeDiscovery/13.3"})
...
def classify(addr,ports,arp=None,http_info=None):
    arp=arp or {}
    vendor=(arp.get("vendor") or "").lower()
    ...
    if 8090 in ports:
        ...
        kind="chromecast"; score=82; name="Chromecast/Google"
    elif 9100 in ports:
    ...
    if "samsung" in text:
        kind="tv_samsung" if kind in ["web_device","device"] else kind
        name=name or "Samsung"
    if "lg" in text:
        kind="tv_lg" if kind in ["web_device","device"] else kind
        name=name or "LG"
...
def scan_ip(addr,arp=None,deep=False):
    ports=FAST_PORTS if not deep else list(PORTS.keys())
    open_ports=[]
    for port in ports:
        if port_open(addr,port):
...
def discovery_scan(limit=None,deep=False):
    with SCAN_LOCK:
        ...
        if not net:
            state.update({"running":False,"progress":100,"message":"Rede não detectada."})
            save(DISCOVERY_STATE,state)
        ...
        def worker(a):
            ...
            with lock:
                ...
                if r:
                    ...
                    partial["devices"]=sorted(results,key=lambda x: tuple(int(p) for p in x["ip"].split(".")))
                    partial["count"]=len(results)
                    partial["message"]=f"Encontrado: {r['name']} ({r['ip']})"
                    save(DISCOVERY_STATE,partial)
                if completed % 8 == 0 or completed==total:
                    partial=load(DISCOVERY_STATE,{})
                    partial["progress"]=min(98,int((completed/total)*100))
                    partial["message"]=f"Verificando rede... {completed}/{total}"
                    save(DISCOVERY_STATE,partial)
        threads=[]
        for a in candidates:
            ...
            th.start()
            threads.append(th)
            while threading.active_count()>42:
                time.sleep(.03)
        for th in threads:
            th.join(timeout=.65)
        ...
        save(NETWORK,{k:v for k,v in payload.items() if k!="ok"})
        save(DISCOVERY_STATE,payload)
        merge_devices([{
            "id":d["id"],
            "source":"discovery",
            "name":d["name"],
...
def discovery_start(limit=None,deep=False):
    ...
    if state.get("running"):
        return {"ok":True,"running":True,"message":"Busca já está em andamento."}
    th=threading.Thread(target=discovery_scan,args=(limit,deep),daemon=True)
    th.start()
    return {"ok":True,"running":True,"message":"Busca iniciada."}

def merge_devices(items):
    cur=load(DEVICES,[])
    by={x.get("id"):x for x in cur}
    for item in items:
        old=by.get(item["id"],{})
        old.update(item)
        by[item["id"]]=old
...
def connect_summary():
    devs=load(DEVICES,[])
    online=[d for d in devs if d.get("online",True)]
    doors=[d for d in devs if d.get("type")=="door"]
    opened=[d for d in doors if d.get("open") in [True,"open","opened",1]]
    ts=[num(d.get("temperature")) for d in online]
    ts=[x for x in ts if x is not None]
    hs=[num(d.get("humidity")) for d in online]
    hs=[x for x in hs if x is not None]
    rooms={}
    for d in devs:
        r=d.get("room") or "Sem ambiente"
        rooms.setdefault(r,{"name":r,"devices":0,"online":0,"temps":[],"hums":[],"doors_open":0})
        rooms[r]["devices"]+=1
        if d.get("online",True):
            rooms[r]["online"]+=1
        if num(d.get("temperature")) is not None:
            rooms[r]["temps"].append(num(d.get("temperature")))
        if num(d.get("humidity")) is not None:
            rooms[r]["hums"].append(num(d.get("humidity")))
        if d.get("type")=="door" and d.get("open") in [True,"open","opened",1]:
            rooms[r]["doors_open"]+=1
    room_list=[]
    for r in rooms.values():
        room_list.append({
            ...
            "avg_temperature":round(sum(r["temps"])/len(r["temps"]),1) if r["temps"] else None,
            "avg_humidity":round(sum(r["hums"])/len(r["hums"]),1) if r["hums"] else None
        })
...
def tuya_host():
    return load(TUYA_CFG,{}).get("data_center","https://openapi.tuyaus.com").rstrip("/")
...
def tuya_request(method,path,body_obj=None,token=None):
    ...
    if not cid or not sec:
        return {"success":False,"msg":"Client ID/Secret não configurados"}
...
def tuya_token():
    cfg=load(TUYA_CFG,{})
    if cfg.get("token") and time.time()<cfg.get("token_expire",0)-60:
        return cfg.get("token")
    res=tuya_request("GET","/v1.0/token?grant_type=1")
    if res.get("success") and res.get("result"):
        cfg["token"]=res["result"].get("access_token","")
        cfg["token_expire"]=time.time()+int(res["result"].get("expire_time",7200))
        save(TUYA_CFG,cfg)
        return cfg["token"]
    return ""
...
def normalize_tuya(d):
    ...
    for k,v in {"mcs":"door","doorcontact":"door","pir":"motion","motion":"motion","wsdcg":"temperature_humidity","temp":"temperature_humidity","humidity":"temperature_humidity","cz":"plug","kg":"switch","light":"light","dj":"light","ipc":"camera","camera":"camera"}.items():
        if k in cat or k in name.lower():
            dtype=v
            break
    ...
    if isinstance(hum,(int,float)) and hum>1000:
        hum=round(hum/10,1)
...
def tuya_sync():
    ...
    if not cfg.get("enabled"):
        return {"ok":False,"message":"Tuya Cloud desativada"}
    token=tuya_token()
    if not token:
        return {"ok":False,"message":"Falha ao obter token Tuya"}
    devices=[]
    asset=cfg.get("asset_id","").strip()
    last={}
...
def health_score(st):
    score=100
    try:
        if int(str(st.get("used","0")).replace("%",""))>85:
            score-=20
    except Exception:
        pass
    if st.get("sdcard")=="não detectado":
        score-=8
    if st["connect"]["total"]==0:
        score-=15
    if st["connect"]["offline"]>0:
        score-=min(20,st["connect"]["offline"]*5)
    if st["connect"]["open_doors"]>0:
        score-=min(15,st["connect"]["open_doors"]*5)
    if st.get("nginx")!="Ativo":
        score-=15
...
def status():
    ...
    d={
        ...
        "storage":stg,
        "sdcard":f"{sd.get('available')} livre de {sd.get('size')}" if sd.get("available") else "não detectado",
        "cpu":cpu(),
        "mem":run("free -m | awk '/Mem:/ {print $3\" MB usado de \"$2\" MB\"}'"),
        "uptime":run("uptime -p"),
        "battery":f"{b.get('percentage')}% • {b.get('status','')}" if b.get("percentage") is not None else "indisponível",
        "temperature":f"{temp}°C" if temp is not None else "indisponível",
        "wifi":w.get("ssid") or "indisponível",
        "wifi_detail":w,
        "connect":con,
        "cameras":cam_counts(),
        "network":net,
        "discovery_state":load(DISCOVERY_STATE,{}),
        "backups":len(list(AREAS["backups"].glob("*"))),
        "tuya_cloud":load(TUYA_CFG,{}),
        "updated":now()
    }
    d["health"]=health_score(d)
    return d
...
def house():
    ...
    con=st["connect"]
    probs=[]
    if st["sdcard"]=="não detectado":
        probs.append("SD Card não detectado")
    if con["total"]==0:
        probs.append("nenhum dispositivo smart home cadastrado")
    if con["offline"]:
        probs.append(f"{con['offline']} dispositivo(s) offline")
    if con["open_doors"]:
        probs.append(f"{con['open_doors']} porta(s)/janela(s) aberta(s)")
...
def jarvis(cmd):
    ...
    st=status()
    hs=house()
    reply="Não entendi. Tente: como está a casa, portas abertas, descobrir rede, sincronizar Tuya."
    action="none"
    if "casa" in t:
        reply=f"Sua casa está com saúde de {st['health']} por cento. {st['connect']['online']} dispositivos online. Temperatura média {hs['temperature'] or 'sem dados'} graus. {hs['message']}."
        ...
        reply="Todas as portas e janelas estão fechadas." if not opens else "Abertas: "+", ".join([x.get("name","Sensor") for x in opens])
        action="open_connect"
    elif "tuya" in t and ("sincron" in t or "buscar" in t):
        r=tuya_sync()
        reply=f"Sincronização Tuya concluída. {r.get('count',0)} dispositivos encontrados." if r.get("ok") else "Não consegui sincronizar Tuya: "+r.get("message","erro")
        action="open_setup"
    elif "rede" in t or "procurar" in t or "scan" in t or "descobrir" in t:
        r=discovery_start()
        reply="Iniciei a descoberta da rede. Abra a tela Discovery para acompanhar em tempo real." if r.get("ok") else r.get("message","Falha na busca")
        action="open_network"
    elif "sensor" in t or "dispositivo" in t:
        c=st["connect"]
        reply=f"Open Home Connect tem {c['total']} dispositivos, {c['online']} online e {c['offline']} offline."
        action="open_connect"
    elif "backup" in t:
        f=backup()
        reply=f"Backup criado: {f}."
    j=load(JARVIS,{"history":[]})
    j.setdefault("history",[]).insert(0,{"command":cmd,"reply":reply,"action":action,"time":now()})
    j["history"]=j["history"][:100]
    save(JARVIS,j)
    ev("jarvis",cmd,"info","jarvis")
...
class H(BaseHTTPRequestHandler):
    ...
    def body(self):
        try:
            return json.loads(self.rfile.read(int(self.headers.get("Content-Length","0"))).decode("utf-8","ignore"))
        except Exception:
            return {}
    ...
    def do_GET(self):
        ...
        routes={
            ...
            "/api/discovery/state":lambda:load(DISCOVERY_STATE,{}),
            ...
            "/api/system/diagnostic":lambda:{"status":status(),"house":house(),"network":load(NETWORK,{}),"discovery_state":load(DISCOVERY_STATE,{}),"tuya":load(TUYA_CFG,{}),"df":run("df -h"),"ip_neigh":run("ip neigh show"),"log":(LOGS/"api.log").read_text(errors="ignore")[-8000:] if (LOGS/"api.log").exists() else ""}
        }
        if u.path in routes:
            return self.js(routes[u.path]())
        self.js({"error":"rota não encontrada"},404)
    def do_POST(self):
        u=urlparse(self.path)
        d=self.body()
        if u.path=="/api/jarvis/command":
            return self.js(jarvis(d.get("command","")))
        if u.path in ["/api/connect/network/scan","/api/discovery/scan"]:
            return self.js(discovery_scan(d.get("limit"),d.get("deep",False)))
        if u.path=="/api/discovery/start":
            return self.js(discovery_start(d.get("limit"),d.get("deep",False)))
        if u.path=="/api/connect/device/add":
            ...
            a.append(d)
            save(DEVICES,a)
            ev("device_add",f"{d.get('name','Dispositivo')} adicionado","success","connect")
        ...
        if u.path=="/api/connect/device/delete":
            save(DEVICES,[x for x in load(DEVICES,[]) if x.get("id")!=d.get("id")])
            return self.js({"ok":True})
        if u.path=="/api/discovery/device/import":
            dev=d.get("device",{})
            if not dev.get("id"):
                return self.js({"ok":False,"message":"Dispositivo inválido"},400)
            item={
                "id":dev.get("id"),
                "source":"discovery",
                "name":dev.get("name") or dev.get("ip"),
                "room":d.get("room") or dev.get("room") or "Rede",
                "type":dev.get("type","device"),
                "online":True,
                "ip":dev.get("ip",""),
                "mac":dev.get("mac",""),
            ...
            merge_devices([item])
            ev("device_import",f"{item['name']} importado para Open Home Connect","success","discovery")
            return self.js({"ok":True,"device":item})
        if u.path=="/api/connect/tuya/config":
            cfg=load(TUYA_CFG,{})
            cfg.update(d)
            cfg["enabled"]=bool(d.get("enabled",cfg.get("enabled",False)))
            save(TUYA_CFG,cfg)
            ev("tuya_config","Configuração Tuya salva","success","connect")
            return self.js({"ok":True,"config":cfg})
        if u.path=="/api/connect/tuya/test":
            cfg=load(TUYA_CFG,{})
            cfg.update(d)
            save(TUYA_CFG,cfg)
            token=tuya_token()
            return self.js({"ok":bool(token),"message":"Conectado à Tuya Cloud" if token else "Falha ao conectar à Tuya Cloud"})
        if u.path=="/api/connect/tuya/sync":
            return self.js(tuya_sync())
        if u.path=="/api/backup":
            return self.js({"ok":True,"file":backup()})
