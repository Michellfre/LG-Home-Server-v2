...
echo "Open Home OS v13.1 Connect Core - Instalando..."
BASE="$HOME/OpenHomeOS"; WEB="$BASE/Web"; CONFIG="$BASE/Config"; DATA="$BASE/Data"
mkdir -p "$WEB" "$CONFIG" "$BASE/Logs" "$DATA/Files" "$DATA/Cameras" "$DATA/Snapshots" "$DATA/Backups" "$DATA/IoT" "$DATA/Connect" "$DATA/Network"
...
[ -f "$CONFIG/network_devices.json" ] || echo '{"last_scan":"","devices":[]}' > "$CONFIG/network_devices.json"
